defmodule ExDocker.Api.LogEntry do
  @moduledoc """
...
defmodule ExDocker.Api.Logs do
  require Logger
  ...
  def stream_logs(host, container_id, opts \\ []) do
    Stream.resource((fn -> start_stream(host, container_id, process_opts(opts)) end),
                    &next/1,
                    &terminate_stream(&1))
  end

  def fetch_logs(host, container_id, opts \\ []) do
    ExDocker.Api.Logs.stream_logs(host, container_id, opts)
    |> Enum.reduce(%{stdout: "", stderr: ""}, fn(%{stream_type: stream_type, message: message}, acc) ->
      Map.update!(acc, String.to_atom(stream_type),  fn(x) -> x <> message end)
    end)
  end

  defp start_stream(host, container_id, opts) do
    {:ok, %HTTPoison.AsyncResponse{id: id}} = HTTPoison.get(
      build_url(host, container_id),
      %{},
      stream_to: self(),
      timeout: :infinity,
  ...
  defp build_url(host, container_id) do
