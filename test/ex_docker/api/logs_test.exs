defmodule ExDocker.Api.LogsTest do
  ...
  setup_all do
    ...
    on_exit("clean_up_image_#{image_name}_#{container_id}", fn ->
      Logger.debug("Deleting image #{image_name} and #{container_id}")
  ...
  test "the log format", context do
    ...
    stream = ExDocker.Api.Logs.stream_logs(@host, cid)
  ...
  defp build_image(name) do
    {:ok, tar_file} = File.read("./test/fixtures/counter/dockerfile.tar.gz")
